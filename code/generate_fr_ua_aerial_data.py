...
MAX_NB_JOBS = min(multiprocessing.cpu_count(),5) # Change to speed up extraction in case multicore architecture is available
...
def generate_urban_atlas_boundaries():
    ...
    ua_gdf.crs = CRS("epsg:3035") #{"init":"epsg:3035"}
    return ua_gdf
...
def main():
    ...
    print("1. Generating Census Data as provided by INSEE...")
    ...
    fn_census = os.path.join(CENSUS_DIR, "squares_to_ses_2019.csv")
    geo_df_data[["idINSPIRE","income"]].to_csv(fn_census,index=False)
    geo_df_data = geo_df_data.to_crs("epsg:3035") #({"init":"epsg:3035"})

    # Extract only images that haven't yet been extracted
    print("2. Extracting only images that haven't yet been extracted...")
    ...
    print("3. Generating Urban Atlas city boundaries to associate each census cell to a city...")
    geo_ua_bound = generate_urban_atlas_boundaries()
    geo_census_ua = gpd.sjoin(geo_df_data,geo_ua_bound).drop("index_right",axis=1)
    extract_ua = geo_census_ua[["idINSPIRE","city"]].copy()
    extract_ua.loc[:,"FUA_NAME"] = [x[0:1].upper() + x[1:].lower() for x in extract_ua.city] ###### CHECK FUA_NAME
    extract_ua[["idINSPIRE","FUA_NAME"]].to_csv(AERIAL_DIR + "city_assoc.csv",index=False)

    # Define Aerial Data tiles as provided by IGN
    print("4. Defining Aerial Data tiles as provided by IGN...")
    geo_aerial_data = generate_aerial_data()
    geo_aerial_data = geo_aerial_data.to_crs("epsg:3035") #({"init":"epsg:3035"})

    # Determine all census cells included in a given aerial tile
    print("5. Determining all census cells included in a given aerial tile...")
    ...
    print("6. Extracting all census cell images from each tile...")
    extract_labelled_aerial_imagery(merge2insee)

    # List all extracted images and associate to their census ID
    print("7. Listing all extracted images and associate to their census ID...")
    ...
    print("8. Excluding any images containing empty pixels...")
